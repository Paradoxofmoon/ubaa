...
fun MainAppScreen(
  ...
  val sportScreens = remember { setOf(AppScreen.SPORT_HOME) }
  ...
  val cardViewModel: CardViewModel? =
      if (currentScreen == AppScreen.CARD) {
        viewModel(key = "card") { CardViewModel() }
      } else {
        null
      }
  val cardUiState = cardViewModel?.state?.collectAsState()?.value ?: CardUiState()

  val networkViewModel: NetworkViewModel? =
      if (currentScreen == AppScreen.NETWORK) {
        viewModel(key = "network") { NetworkViewModel() }
      } else {
        null
      }
  ...
  val zfwViewModel: ZfwViewModel? =
      if (currentScreen == AppScreen.ZFW) {
        viewModel(key = "zfw") { ZfwViewModel() }
      } else {
        null
      }
  ...
  LaunchedEffect(currentScreen, shouldKeepCgyyViewModel) {
    ...
    when (currentScreen) {
      ...
      AppScreen.CARD -> cardViewModel?.ensureLoaded()
      AppScreen.NETWORK -> networkViewModel?.ensureLoaded()
      AppScreen.ZFW -> Unit
      AppScreen.ELECTRICITY -> Unit
      AppScreen.YGDK_HOME,
        ...
        when (currentScreen) {
          ...
          AppScreen.REGULAR ->
              RegularFeaturesScreen(
                  ...
                  onCardClick = { navigateTo(AppScreen.CARD) },
