...
import cn.edu.ubaa.api.feature.RelayCardApiBackend
...
import cn.edu.ubaa.api.feature.RelayNetworkApiBackend
...
import cn.edu.ubaa.api.feature.RelayZfwApiBackend
...
internal object DefaultApiFactory : ApiFactory {
  ...
  override fun cardApi(): CardApiBackend =
      when (mode()) {
        ConnectionMode.DIRECT -> localBackends(ConnectionMode.DIRECT).cardApi
        ConnectionMode.WEBVPN -> localBackends(ConnectionMode.WEBVPN).cardApi
        ConnectionMode.SERVER_RELAY -> RelayCardApiBackend()
      }

  override fun networkApi(): NetworkApiBackend =
      when (mode()) {
        ConnectionMode.DIRECT -> localBackends(ConnectionMode.DIRECT).networkApi
        ConnectionMode.WEBVPN -> localBackends(ConnectionMode.WEBVPN).networkApi
        ConnectionMode.SERVER_RELAY -> RelayNetworkApiBackend()
      }

  override fun zfwApi(): ZfwApiBackend =
      when (mode()) {
        ConnectionMode.DIRECT -> localBackends(ConnectionMode.DIRECT).zfwApi
        ConnectionMode.WEBVPN -> localBackends(ConnectionMode.WEBVPN).zfwApi
        ConnectionMode.SERVER_RELAY -> RelayZfwApiBackend()
      }
...
internal object RelayApiFactory : ApiFactory {
  ...
  override fun cardApi(): CardApiBackend = RelayCardApiBackend()

  override fun networkApi(): NetworkApiBackend = RelayNetworkApiBackend()

  override fun zfwApi(): ZfwApiBackend = RelayZfwApiBackend()
}
