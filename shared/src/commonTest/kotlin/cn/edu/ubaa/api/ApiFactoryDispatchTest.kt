...
class ApiFactoryDispatchTest {
  ...
  @Test
  fun `card api default constructor delegates to connection runtime api factory`() = runTest {
    val backend = FakeCardApiBackend()
    ConnectionRuntime.apiFactoryProvider = {
      FakeApiFactory(
          cardBackend = backend,
      )
    ...
    val result = CardApi().getBalance()

    assertTrue(result.isSuccess)
    assertEquals(1, backend.getBalanceCalls)
  }

  @Test
  fun `network api default constructor delegates to connection runtime api factory`() = runTest {
    val backend = FakeNetworkApiBackend()
    ConnectionRuntime.apiFactoryProvider = {
      FakeApiFactory(
    ...
    val result = NetworkApi().getTraffic()

    assertTrue(result.isSuccess)
    assertEquals(1, backend.getTrafficCalls)
  }
  ...
  fun `default api factory routes webvpn supported services to local backends`() {
    ...
    assertTrue(DefaultApiFactory.cardApi() is LocalCardApiBackend)
    assertTrue(DefaultApiFactory.networkApi() is LocalNetworkApiBackend)
  }
  ...
  fun `default api factory reuses local backends within selected mode`() {
    ...
    val directBackends =
        listOf(
            ...
            DefaultApiFactory.cardApi() to DefaultApiFactory.cardApi(),
            DefaultApiFactory.networkApi() to DefaultApiFactory.networkApi(),
        )
...
private class FakeApiFactory(
  ...
  override fun cardApi(): CardApiBackend = cardBackend
...
private class FakeCardApiBackend : CardApiBackend {
...
private class FakeNetworkApiBackend : NetworkApiBackend {
  var getTrafficCalls = 0
    private set

  override suspend fun getTraffic(): Result<TrafficData> {
    getTrafficCalls++
    return Result.success(TrafficData(freeTrafficTotal = 30.0, freeTrafficRemaining = 20.0))
  }
}
